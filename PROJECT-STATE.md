...
- 流程：节点“完成”时重新校验前置依赖；撤回上游前检查直接下游，下游已开始/完成时拒绝撤回，避免前置未完成但下游推进的脏状态
- 修复：撤回（清空实际完成日期）先做部门/角色权限校验，非本部门用户不能撤回他人节点
...
- 测试：API 72 项、后端单元 103 项、前端 Vitest 20 项、压力测试（并发建单/阶段推进/50 单导出）、E2E、依赖审计
