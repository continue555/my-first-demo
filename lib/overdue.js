function getOverdueInfo(obj) {
  ...
  if (status !== 'completed' && status !== 'cancelled' && planned) {
  ...
  if (status === 'completed' && planned && actual) {
    const p = planned.includes('T') ? planned : planned.slice(0, 10);
    const a = actual.includes('T') ? actual : actual.slice(0, 10);
    if (a > p) {
      return { cssClass: 'overdue-red', rowClass: 'overdue-red-row', stampClass: 'overdue-red-stamp', stampText: '超期' };
    }
    if (a <= p) {
